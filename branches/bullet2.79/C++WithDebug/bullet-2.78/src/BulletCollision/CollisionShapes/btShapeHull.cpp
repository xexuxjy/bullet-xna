...
btShapeHull::buildHull (btScalar /*margin*/)
{
	...
	if(g_file && btBulletDebug::debugShapeHull)
	{
		fprintf(g_file,"buildHull\n");
		for(int i=0;i<m_vertices.size();++i)
		{
			btGeometryUtil::PrintVector(g_file,m_vertices[i]);
		}
		fprintf(g_file,"Indices\n");
		for (int i = 0; i < m_indices.size()/3; ++i)
		{
			int indexer = i*3;
			fprintf(g_file,"%d,%d,%d\n", m_indices[indexer], m_indices[indexer + 1], m_indices[indexer + 2]);
		}
	}
