...
#define EPA_INSIDE_EPS		((btScalar)0.01)
	...
	struct	GJK
	{
		...
		struct	eStatus	{ enum _ {
			...
			eStatus::_			Evaluate(const tShape& shapearg,const btVector3& guess)
			{
				...
				if (g_file && btBulletDebug::debugGJK)
				{
					fprintf(g_file,"gjk eval dist[%2.8f]\n", m_distance);
				}
		...
		bool withmargins)
	{
		...
		if (g_file && btBulletDebug::debugGJK)
		{
			btGeometryUtil::PrintMatrix(g_file, "gjksolver2::init::shape0", shape.m_toshape0);
			btGeometryUtil::PrintMatrix(g_file, "gjksolver2::init::WTRS0", wtrs0);
			btGeometryUtil::PrintMatrix(g_file, "gjksolver2::init::WTRS1", wtrs1);
			btGeometryUtil::PrintMatrix(g_file, "gjksolver2::init::shape1", shape.m_toshape1);
		}
