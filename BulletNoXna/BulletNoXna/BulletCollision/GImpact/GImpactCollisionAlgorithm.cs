...
namespace BulletXNA.BulletCollision
{
    ...
    public static class PlaneShape
    {
        ...
        {
            equation = new Vector4(plane.GetPlaneNormal(),plane.PlaneConstant);
        ...
        public static void GetPlaneEquationTransformed(StaticPlaneShape plane,ref Matrix trans, out Vector4 equation)
        {
            ...
            equation.W = trans.Translation.Dot(ref planeNormal) + plane.PlaneConstant;
        }
    }
    ...
    public class GImpactCollisionAlgorithm : ActivatingCollisionAlgorithm
    {
        ...
        {
            CollisionShape tmpShape0 = body0.CollisionShape;
            CollisionShape tmpShape1 = body1.CollisionShape;
                          ...
                          ObjectArray<int> collided_primitives)
        {
            ...
            if (shape0.HasBoxSet())
            {
                Matrix trans1to0 = trans0.Inverse();
                //trans1to0 *= trans1;
                          ...
                          StaticPlaneShape shape1, bool swapped)
        {
            ...
            tribox.IncrementMargin(shape1.Margin);

            if (tribox.PlaneClassify(ref plane) != BT_PLANE_INTERSECTION_TYPE.BT_CONST_COLLIDE_PLANE) return;

            shape0.LockChildShapes();

            float margin = shape0.Margin + shape1.Margin;

            Vector3 vertex;
            int vi = shape0.VertexCount;
            while (vi-- != 0)
            {
                shape0.GetVertex(vi, out vertex);
                vertex = orgtrans0 * vertex;

                float distance = Vector3.Dot(vertex, MathUtil.Vector4ToVector3(ref plane)) - plane.W - margin;
        ...
        public override void ProcessCollision(CollisionObject body0, CollisionObject body1, DispatcherInfo dispatchInfo, ManifoldResult resultOut)
        {
            ...
            if (body0.CollisionShape.ShapeType == BroadphaseNativeType.GImpactShape)
            {
                gimpactshape0 = body0.CollisionShape as GImpactShapeInterface;

                if (body1.CollisionShape.ShapeType == BroadphaseNativeType.GImpactShape)
                {
            ...
            else if (body1.CollisionShape.ShapeType == BroadphaseNativeType.GImpactShape)
            {
                gimpactshape1 = body1.CollisionShape as GImpactShapeInterface;
        ...
        public static void RegisterAlgorithm(CollisionDispatcher dispatcher)
        {
            ...
            for (i = 0; i < (int)BroadphaseNativeType.MAX_BROADPHASE_COLLISION_TYPES; i++)
            {
                dispatcher.RegisterCollisionCreateFunc((int)BroadphaseNativeType.GImpactShape, i, s_gimpact_cf);
            }

            for (i = 0; i < (int)BroadphaseNativeType.MAX_BROADPHASE_COLLISION_TYPES; i++)
            {
                dispatcher.RegisterCollisionCreateFunc(i, (int)BroadphaseNativeType.GImpactShape, s_gimpact_cf);
            }
	...
	if(shape0.GetGImpactShapeType() == GIMPACT_SHAPE_TYPE.CONST_GIMPACT_TRIMESH_SHAPE_PART &&
		shape1.ShapeType == BroadphaseNativeType.StaticPlane)
	{
                          ...
                          ConcaveShape shape1, bool swapped)
        {
            ...
            tricallback.margin = shape1.Margin;
