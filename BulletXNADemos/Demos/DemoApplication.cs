...
namespace BulletXNADemos.Demos
{
	public class DemoApplication : Microsoft.Xna.Framework.Game
	{
		...
		public const float STEPSIZE = MathUtil.SIMD_PI / 3f; // 60 deg a second
		...
		public virtual void KeyboardCallback(Keys key, int x, int y, GameTime gameTime, bool released, ref KeyboardState newState, ref KeyboardState oldState)
		{
			...
			switch (key)
			{
				...
				case Keys.L: StepLeft(gameTime); break;
				case Keys.R: StepRight(gameTime); break;
				case Keys.F: StepFront(gameTime); break;
				case Keys.B: StepBack(gameTime); break;
				case Keys.Z: ZoomIn(gameTime); break;
				case Keys.X: ZoomOut(gameTime); break;
				...
				case Keys.Left: StepLeft(gameTime); break;
				case Keys.Right: StepRight(gameTime); break;
				case Keys.Up: StepFront(gameTime); break;
				case Keys.Down: StepBack(gameTime); break;
				case Keys.PageUp: ZoomIn(gameTime); break;
				case Keys.PageDown: ZoomOut(gameTime); break;
		...
		{
			float delta = (STEPSIZE * (float)gameTime.ElapsedGameTime.TotalSeconds);
			m_yaw -= delta;
			//if (m_yaw < 0)
			//{
		...
		public void StepFront(GameTime gameTime)
		{
			float delta = (STEPSIZE * (float)gameTime.ElapsedGameTime.TotalSeconds);
		...
		public void StepBack(GameTime gameTime)
		{
			float delta = (STEPSIZE * (float)gameTime.ElapsedGameTime.TotalSeconds);
