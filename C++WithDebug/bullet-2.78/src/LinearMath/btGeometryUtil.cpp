...
void btGeometryUtil::PrintVector(FILE *file ,const btVector3& v)
{
	fprintf(file,"{X:%4.8f Y:%4.8f Z:%4.8f}\n",v.x(),v.y(),v.z());
}

void btGeometryUtil::PrintVector(FILE *file ,const char* name,const btVector3& v)
{
	fprintf(file,"[%s] {X:%4.8f Y:%4.8f Z:%4.8f}\n",name,v.x(),v.y(),v.z());
}

void btGeometryUtil::PrintMatrix(FILE* file,const btMatrix3x3& m)
{
	PrintMatrix(file,NULL,m);
}
...
void btGeometryUtil::PrintMatrix(FILE* file,const char* name,const btTransform& m)
{
	const btMatrix3x3& basis = m.getBasis();
	const btVector3 right = basis.getColumn(0);
	const btVector3 up = basis.getColumn(1);
	const btVector3 backward = basis.getColumn(2);
	const btVector3 translation = m.getOrigin();
	if(name)
	{
		fprintf(file,"%s\n",name);
	}
	...
	PrintVector(file,"Up          ",up);
	PrintVector(file,"Backward    ",backward);
	PrintVector(file,"Translation ",translation);
}

void btGeometryUtil::PrintQuaternion(FILE* file,const btQuaternion& q)
{
	fprintf(file,"{X:%4.8f Y:%4.8f Z:%4.8f W:%4.8f}",q.x(),q.y(),q.z(),q.w());
}
