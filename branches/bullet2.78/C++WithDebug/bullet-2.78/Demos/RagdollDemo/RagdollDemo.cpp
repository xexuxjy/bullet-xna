...
class RagDoll
{
	...
	btRigidBody* localCreateRigidBody (btScalar mass, const btTransform& startTransform, btCollisionShape* shape)
	{
		...
		fprintf(g_file,"localCreateRigidBody [%d] startTransform\n",body->m_debugBodyId);
		btGeometryUtil::PrintMatrix(g_file,startTransform);
...
public:
	RagDoll (btDynamicsWorld* ownerWorld, const btVector3& positionOffset,FILE* file)
		: m_ownerWorld (ownerWorld)
	{
		...
		m_bodies[BODYPART_PELVIS]->setUserPointer("PELVIS");

		transform.setIdentity();
		transform.setOrigin(btVector3(btScalar(0.), btScalar(1.2), btScalar(0.)));
		m_bodies[BODYPART_SPINE] = localCreateRigidBody(btScalar(1.), offset*transform, m_shapes[BODYPART_SPINE]);
		m_bodies[BODYPART_SPINE]->setUserPointer("SPINE");

		transform.setIdentity();
		transform.setOrigin(btVector3(btScalar(0.), btScalar(1.6), btScalar(0.)));
		m_bodies[BODYPART_HEAD] = localCreateRigidBody(btScalar(1.), offset*transform, m_shapes[BODYPART_HEAD]);
		m_bodies[BODYPART_HEAD]->setUserPointer("HEAD");

		transform.setIdentity();
		transform.setOrigin(btVector3(btScalar(-0.18), btScalar(0.65), btScalar(0.)));
		m_bodies[BODYPART_LEFT_UPPER_LEG] = localCreateRigidBody(btScalar(1.), offset*transform, m_shapes[BODYPART_LEFT_UPPER_LEG]);
		m_bodies[BODYPART_LEFT_UPPER_LEG]->setUserPointer("LEFTUPPERLEG");

		transform.setIdentity();
		transform.setOrigin(btVector3(btScalar(-0.18), btScalar(0.2), btScalar(0.)));
		m_bodies[BODYPART_LEFT_LOWER_LEG] = localCreateRigidBody(btScalar(1.), offset*transform, m_shapes[BODYPART_LEFT_LOWER_LEG]);
		m_bodies[BODYPART_LEFT_LOWER_LEG]->setUserPointer("LEFTLOWERLEG");

		transform.setIdentity();
		transform.setOrigin(btVector3(btScalar(0.18), btScalar(0.65), btScalar(0.)));
		m_bodies[BODYPART_RIGHT_UPPER_LEG] = localCreateRigidBody(btScalar(1.), offset*transform, m_shapes[BODYPART_RIGHT_UPPER_LEG]);
		m_bodies[BODYPART_RIGHT_UPPER_LEG]->setUserPointer("RIGHTUPPERLEG");

		transform.setIdentity();
		transform.setOrigin(btVector3(btScalar(0.18), btScalar(0.2), btScalar(0.)));
		m_bodies[BODYPART_RIGHT_LOWER_LEG] = localCreateRigidBody(btScalar(1.), offset*transform, m_shapes[BODYPART_RIGHT_LOWER_LEG]);
		m_bodies[BODYPART_RIGHT_LOWER_LEG]->setUserPointer("RIGHTLOWERLEG");

		transform.setIdentity();
		transform.setOrigin(btVector3(btScalar(-0.35), btScalar(1.45), btScalar(0.)));
		transform.getBasis().setEulerZYX(0,0,M_PI_2);
		m_bodies[BODYPART_LEFT_UPPER_ARM] = localCreateRigidBody(btScalar(1.), offset*transform, m_shapes[BODYPART_LEFT_UPPER_ARM]);
		m_bodies[BODYPART_LEFT_UPPER_ARM]->setUserPointer("LEFTUPPERARM");

		transform.setIdentity();
		transform.setOrigin(btVector3(btScalar(-0.7), btScalar(1.45), btScalar(0.)));
		transform.getBasis().setEulerZYX(0,0,M_PI_2);
		m_bodies[BODYPART_LEFT_LOWER_ARM] = localCreateRigidBody(btScalar(1.), offset*transform, m_shapes[BODYPART_LEFT_LOWER_ARM]);
		m_bodies[BODYPART_LEFT_LOWER_ARM]->setUserPointer("LEFTLOWERARM");

		transform.setIdentity();
		transform.setOrigin(btVector3(btScalar(0.35), btScalar(1.45), btScalar(0.)));
		transform.getBasis().setEulerZYX(0,0,-M_PI_2);
		m_bodies[BODYPART_RIGHT_UPPER_ARM] = localCreateRigidBody(btScalar(1.), offset*transform, m_shapes[BODYPART_RIGHT_UPPER_ARM]);
		m_bodies[BODYPART_RIGHT_UPPER_ARM]->setUserPointer("RIGHTUPPERARM");

		transform.setIdentity();
		transform.setOrigin(btVector3(btScalar(0.7), btScalar(1.45), btScalar(0.)));
		transform.getBasis().setEulerZYX(0,0,-M_PI_2);
		m_bodies[BODYPART_RIGHT_LOWER_ARM] = localCreateRigidBody(btScalar(1.), offset*transform, m_shapes[BODYPART_RIGHT_LOWER_ARM]);
		m_bodies[BODYPART_RIGHT_LOWER_ARM]->setUserPointer("RIGHTLOWERARM");
...
void RagdollDemo::spawnRagdoll(const btVector3& startOffset,FILE* file)
{
	RagDoll* ragDoll = new RagDoll (m_dynamicsWorld, startOffset,file);
