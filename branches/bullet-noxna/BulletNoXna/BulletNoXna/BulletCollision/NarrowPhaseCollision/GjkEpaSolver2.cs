...
namespace BulletXNA.BulletCollision
{
    public class GjkEpaSolver2
    {
        ...
        public static bool Penetration(ConvexShape shape0, ref Matrix wtrs0, ConvexShape shape1, ref Matrix wtrs1, ref Vector3 guess, ref GjkEpaSolver2Results results)
        {
        ...
        //
        public float SignedDistance(ref Vector3 position, float margin, ConvexShape shape0, ref Matrix wtrs0, ref GjkEpaSolver2Results results)
        {
            using (GjkEpaSolver2MinkowskiDiff shape = BulletGlobals.GjkEpaSolver2MinkowskiDiffPool.Get())
            using (GJK gjk = BulletGlobals.GJKPool.Get())
            {
    ...
    public class GJK : IDisposable
    {
        ...
        public GJKStatus Evaluate(GjkEpaSolver2MinkowskiDiff shapearg, ref Vector3 guess)
        {
            ...
            do	
            {
                ...
                float omega=Vector3.Dot(ref m_ray,ref w)/rl;
        ...
        public static float ProjectOrigin(ref Vector3 a,ref Vector3 b,ref Vector4 w,ref uint m)
        {
            ...
            {
                float t = (l>0f?(-Vector3.Dot(ref a,ref d)/l):0f);
                if(t>=1)		
                { 
                    w.X=0f;
                    w.Y=1f;
        ...
        static Vector3[] vt = new Vector3[3];
        static Vector3[] dl = new Vector3[3];
            ...
            ref Vector4 w,ref uint m)
        {
            ...
            if (l > GjkEpaSolver2.GJK_SIMPLEX3_EPS)
            {
                ...
                if(mindist<0)
                {
                    float d = Vector3.Dot(ref a,ref n);	
        ...
        private static Vector3[] vta = new Vector3[4];
        private static Vector3[] dla = new Vector3[3];
                ...
                if(m_hull.Count==4)
                {
                    ...
                    for (; iterations < GjkEpaSolver2.EPA_MAX_ITERATIONS; ++iterations)
                    {
                        if (m_nextsv < GjkEpaSolver2.EPA_MAX_VERTICES)
                        {
                            ...
                            float wdist=Vector3.Dot(ref best.n,ref w.w)-best.d;
        ...
        public sFace NewFace(sSV a,sSV b,sSV c,bool forced)
        {
            if(m_stock.Count > 0)
            {
                ...
                if(v)
                {
                    face.d = Vector3.Dot(ref a.w,ref face.n)/l;
                    face.n /= l;
        ...
        public bool Expand(uint pass, sSV w, sFace f, uint e, ref sHorizon horizon)
        {
            ...
            {
                uint e1 = i1m3[e];
                if ((Vector3.Dot(ref f.n, ref w.w) - f.d) < -GjkEpaSolver2.EPA_PLANE_EPS)
                {
