...
void	btQuantizedBvh::reportAabbOverlappingNodex(btNodeOverlapCallback* nodeCallback,const btVector3& aabbMin,const btVector3& aabbMax) const
{
	...
	if (m_useQuantization)
	{
		...
		if(g_file && debugQuantizedBVH)
		{
			fprintf(g_file,"reportAabbOverlappingNodex\n");
			btGeometryUtil::PrintVector(g_file," aabbMin",aabbMin);
			btGeometryUtil::PrintVector(g_file,"aabbMax",aabbMax);
			fprintf(g_file,"quantizedMin [%d][%d][%d]\n",quantizedQueryAabbMin[0],quantizedQueryAabbMin[1],quantizedQueryAabbMin[2]);
			fprintf(g_file,"quantizedMax [%d][%d][%d]\n",quantizedQueryAabbMax[0],quantizedQueryAabbMax[1],quantizedQueryAabbMax[2]);
