...
void	btSequentialImpulseConstraintSolver::convertContact(btPersistentManifold* manifold,const btContactSolverInfo& infoGlobal)
{
	...
	for (int j=0;j<manifold->getNumContacts();j++)
	{
		...
		if (cp.getDistance() <= manifold->getContactProcessingThreshold())
		{
			...
			if(rb0 && g_file && btBulletDebug::debugSolver)
			{
				fprintf(g_file,"ConvertContact [%s][%s]\n", (char*)solverConstraint.m_solverBodyA->getUserPointer(), (char*)solverConstraint.m_solverBodyB->getUserPointer());
				btManifoldPoint::PrintContactPoint(g_file,cp);
			}
			...
			{
				btTypedConstraint::PrintSolverConstraint(g_file,&solverConstraint,99);
			}
	...
	{
		{
			...
			for (i=0;i<numConstraints;i++)
			{
				...
				if (info1.m_numConstraintRows)
				{
					...
					info2.numRows = info1.m_numConstraintRows;
					...
					for ( j=0;j<info1.m_numConstraintRows;j++)
					{
						...
						if(g_file && btBulletDebug::debugSolver)
						{
							btTypedConstraint::PrintSolverConstraint(g_file,&solverConstraint,j);
