...
namespace BulletXNA.BulletDynamics
{
    public class RaycastVehicle : IActionInterface
    {
        ...
        {
            /*if (getRigidBody()->MotionState)
            {
                btTransform chassisWorldTrans;
                getRigidBody()->MotionState->getWorldTransform(chassisWorldTrans);
        ...
        public virtual void UpdateVehicle(float step)
        {
	        ...
	        m_currentVehicleSpeedKmHour = 3.6f * GetRigidBody().LinearVelocity.Length();
	        ...
	        if (Vector3.Dot(forwardW,GetRigidBody().LinearVelocity) < 0f)
	        {
        ...
        //	up.normalize();
	        ...
	        Quaternion steeringOrn = new Quaternion(up,steering);//wheel.m_steering);
            IndexedBasisMatrix steeringMat = new IndexedBasisMatrix(ref steeringOrn);

            Quaternion rotatingOrn = new Quaternion(right, -wheel.m_rotation);
        ...
        public void UpdateWheelTransformsWS(WheelInfo wheel, bool interpolatedTransform)
        {
            ...
            Matrix chassisTrans = GetChassisWorldTransform();
            if (interpolatedTransform && (GetRigidBody().MotionState != null))
            {
                GetRigidBody().MotionState.GetWorldTransform(out chassisTrans);
            }
            ...
            // apply the impulses
            {
	            for (int wheel = 0;wheel<numWheel ; wheel++)
	            {
                    ...
                    if (m_sideImpulse[wheel] != 0f)
                    {
                        ...
                        Vector3 vChassisWorldUp = GetRigidBody().GetCenterOfMassTransform()._basis.GetColumn(m_indexUpAxis);
